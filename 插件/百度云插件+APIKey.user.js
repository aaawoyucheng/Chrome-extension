 ...
 */
    ...
    request:function(){
        ...
        GM_xmlhttpRequest({
                    ...
                    onload: function(response) {
                        ...
                        _this.totalPage=(parseInt(_this.jsonObj.cursor.resultCount)-parseInt(_this.jsonObj.cursor.resultCount)%20)/20+1;
                        _this.state=true;
                        var _self=_this;
 ...
 Viewer.prototype={
    ...
    refleshPageNavi:function(){
        ...
        page.totalPage=this.model.totalPage>=20?20:this.model.totalPage;
 ...
 function initial(){
    ...
    bdModel.urls.google='https://www.googleapis.com/customsearch/v1element?key=AIzaSyCVAXiUzRYsML1Pv6RwSG1gunmMikTzQqY&rsz=filtered_cse&num=20&hl=en&prettyPrint=true&source=gcsc&gss=.com&sig=ee93f9aae9c9e9dba5eea831d506e69a&cx=018177143380893153305:yk0qpgydx_e&q=';
    ...
    bdModel.toJson.bing=function(html){
        ...
        if(matchLst!==null){//匹配100,000,111之类的情况
            ...
            if(matchLst!==null){//匹配20 个结果之类的情况，以及1-11，共11个的情况
            data.cursor.resultCount=matchLst.pop();
            }else{//匹配无的情况
            data.cursor.resultCount=0;
            }
        }
        data.cursor.resultCount = parseInt( data.cursor.resultCount.toString(),20);
    ...
    bdModel.compileUrl.bing=function(_self){
        return _self.urls.bing + _self.keyword + '+site%3Apan.baidu.com' + '&first=' + (_self.curr-1)*20;
    };
    bdModel.compileUrl.google=function(_self){
        return _self.urls.google + _self.keyword + '&start=' + (_self.curr-1)*20;
    };
