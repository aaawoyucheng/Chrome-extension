...
$(function () {
	...
	chrome.storage.local.get(null, function (items) {
		...
		if (!jQuery.isEmptyObject(items.sites)) {
			(function () {
				for (var key in items.sites) {
					var response = items.sites[key];
					var js = btnConstructor(response.js, 'js', 'JavaScript'),
				...
				// Modal window
				var modal = new ModalWorker();

				// Close modal
				$('#popupFrame').on("click", '.popupWrapper', function (e) {
					if (e.target.className == 'popupWrapper') modal.closeModal();
				});
				$('#popupFrame').on("click", '#btn_close', function () {
					modal.closeModal();
				});
...
function updateLibsList(items) {
	chrome.storage.local.get(null, function (items) {
		...
		if (!jQuery.isEmptyObject(items.libs)) {
			var tSrc = chrome.i18n.getMessage('src'),
			    tOrder = chrome.i18n.getMessage('order'),
			    _tNoLibs = chrome.i18n.getMessage('no_libs');
			$('#libsList').empty();
			for (var key in items.libs) {
				var lib = items.libs[key],
