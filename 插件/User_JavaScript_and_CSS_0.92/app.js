...
var ModalWorker = function() {
    ...
    ModalWorker.prototype.openModal = function openModal(domain, type) {
        ...
        else editor.getSession().setMode("ace/mode/css");

        chrome.storage.local.get(null, function(data) {
            if (data.sites[domain]) {
                var response = data.sites[domain];
                if (type == 'js') editor.setValue(response.js);
                else editor.setValue(response.css);
            }
        });
...
function migrateToNewVersion(callback) {
    chrome.storage.local.get(null, function(items) {
        ...
        var data = {
            sites: {},
            libs: {
                'jQuery 1.12.4': {get src() {
                        return chrome.extension.getURL('js/jquery/1.12.4/jquery.min.js');
                    },
                    order: 5
                },
                'jQuery 2.2.4': {get src() {
                        return chrome.extension.getURL('js/jquery/2.2.4/jquery.min.js');
                    },
                    order: 5
                },
                'jQuery 3.1.1': {get src() {
                        return chrome.extension.getURL('js/jquery/3.1.1/jquery.min.js');
                    },
                    order: 5
                }
            }
